...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.faqItem.create({ data: { ...parsed.data, order: count } });

  const { ipAddress, userAgent } = await getRequestMeta();
  await logAuditEvent({ userId: admin.id, action: "FAQ_CREATED", entity: "FaqItem", entityId: item.id, ipAddress, userAgent });
